...
const page = async ({params}: PageProps) => {
  ...
  return <div className='flex-1 justify-between flex flex-col h-full max-h-[calc(100vh-6rem)]'>
    <div className='flex sm:items-center justify-between py-3 border-b-2 border-gray-200 dark:border-gray-900'>
      <div className=' relative flex items-center space-x-4'>
        ...
        <div className='flex flex-col leading-tight'>
          <div className='text-xl flex items-center'>
            <span className='text-gray-700 mr-3 font-semibold dark:text-violet-600'>{chatPartner.name}</span>
          </div>

          <span className='text-sm text-gray-600 dark:text-gray-500'>{chatPartner.email}</span>
        </div>
      </div>
    </div>
